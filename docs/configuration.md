...
- At runtime, `WM1303Backend.get_radios()` reads channel definitions **primarily from `wm1303_ui.json`** (the SSOT). Channels defined in `config.yaml` are only used as a **fallback** when no active channels exist in the SSOT.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `identity_key` | string | auto-generated | Unique 32-byte mesh identity key, generated by the install script using `secrets.token_bytes(32)`. Preserved on upgrade. Do not edit manually |
...
| `identity_key` | string | auto-generated | Unique 32-byte repeater identity key, generated by the install script using `secrets.token_bytes(32)`. Preserved on upgrade. Do not edit manually |
...
Defines the LoRa channels available for the bridge. Each channel is identified by a key (e.g., `channel_a`, `channel_b`). **Note:** These channel definitions serve as a **fallback only**. At runtime, `WM1303Backend.get_radios()` reads channels primarily from `wm1303_ui.json` (the SSOT). Config.yaml channels are only used when no active channels are defined in the SSOT.
...
The following values are automatically generated during installation if not present in `config.yaml`. Both `install.sh` and `upgrade.sh` check for these keys and generate them if missing, using Python's `secrets.token_bytes(32)` for identity keys and `secrets.token_hex()` for other secrets. Existing values are always preserved.

| Secret | Purpose | Generation |
|--------|---------|------------|
| `mesh.identity_key` | Unique mesh network identity | 32-byte cryptographic random via `secrets.token_bytes(32)` |
| `repeater.identity_key` | Repeater identity for the network | 32-byte cryptographic random via `secrets.token_bytes(32)` |
